...
void MoveRing(int n, int from, int pass, int to, vector<vector<int>>& history) {
	if (n == 1) {
		history.emplace_back(vector<int>{from, to});
		return;
	}
	MoveRing(n - 1, from, to, pass, history);
	history.emplace_back(vector<int>{from, to});
	MoveRing(n - 1, pass, from, to, history);

}

vector<vector<int>> ComputeTowerHanoi(int num_rings) {
	vector<vector<int>> result;
	MoveRing(num_rings, 0, 2, 1, result);
	return result;
}
